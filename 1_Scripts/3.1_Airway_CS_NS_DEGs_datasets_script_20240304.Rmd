---
title: "3.1_Airway_CS_NS_DEG_script_20240304"
...
samples_GSE4302 <- strsplit("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX100X0X000XXX000X000X0XXX001110001100000001111101101110", "")[[1]]
length(samples_GSE4302=="X")
...
## Looking at age and packyears
phenotypic_data_A2 <- pData(gset)

# Age
age_a2 <- gsub("Age:", "", phenotypic_data_A2$`characteristics_ch1.1`)
age_a2 <- as.numeric(gsub(" Packyears:.*", "", age_a2))
median(age_a2)
range(age_a2)

# Packyears
packyears_a2 <- ifelse(
  grepl("Packyears:", phenotypic_data_A2$`characteristics_ch1.1`),
  as.numeric(sub(".*Packyears:([0-9]+\\.?[0-9]*).*", "\\1", phenotypic_data_A2$`characteristics_ch1.1`)),
  0
)
median(packyears_a2[packyears_a2>0])
range(packyears_a2[packyears_a2>0])
```
...
samples_GSE19027 <- strsplit("11000000X1XX011X11111XXXXXXXXXXXX111111XX11XXXXXX1X1010XXXXX", "")[[1]]
length(samples_GSE19027=="X")
...
length(strsplit("XXXXXXXXXXXXXXXXX1100000000XXXXXXXXXXX11111111000011111XXXXXXX0000000000011111111111111111100010010011XXXX1100011000011111111111X000001","")[[1]]=="X")

A5_phenotypic_data <- pData(gset)

# Age
a5_age <- as.numeric(gsub("age: ", "", A5_phenotypic_data$characteristics_ch1))
a5_age[6] <- 45 ## The "Age:" label threw it off
median(a5_age)
range(a5_age)

# Sex
table(A5_phenotypic_data$characteristics_ch1.1)

# Ethnicity
table(A5_phenotypic_data$characteristics_ch1.2)

# Packyears
A5_packyears <- as.numeric(ifelse(
  grepl("pack-years", A5_phenotypic_data$characteristics_ch1.3),
  sub(".*?(\\d+\\.?\\d*)\\s*pack-years.*", "\\1", A5_phenotypic_data$characteristics_ch1.3),
  0
))
median(A5_packyears[A5_packyears>0])
range(A5_packyears[A5_packyears>0])
