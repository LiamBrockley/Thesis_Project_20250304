---
title: "1_Tumor_airway_comparisons_differential_analysis_LUAD_CSFSNS_sanity_checks_20250304"
...
In this version of the main comparisons in my pipeline, I am using TCGA-LUAD and including never smokers so that I can compare the outputs with the outputs from other datasets and studies that compared tumor and normal tissue and including all smoking statuses.
...
2025/03/05: It seems that the expression values of the counts file indicate that the counts are actually raw and not RMA normalized. So, I am going to try this again where I download the raw .CEL files and normalize them with RMA myself. Maybe the batch effect will also be gone if I do it this way... let's see. [[ Try this when I have better wifi...]]

```{r}

# getGEOSuppFiles("GSE63127")
# 
# library(affy)
# raw_data <- ReadAffy(celfile.path = "GSE63127")
...
#write.table(GSE63127_CS_NS_limma, "../2_Outputs/1_Airway_DEGs/GSE63127_CS_NS_limma_20250305.txt", sep = '\t')
...
## NOTE: Should do normalization before hierarchical clustering - TPM
## For PCA I should do normalization as well

# SummarizedExperiment
# vst does the normalization
...
## Redo with the normaliztion and check the blacklist as well
...
legend("bottomleft", 
       legend = c("Tumor", "Normal"), counts_matched_T_NM, 
...
Note below that I include a filtering step to remove genes with low counts:
...
### Low read counts filtering #####
keep <- filterByExpr(counts_matched_T_NM, design = design) # new method
#keep <- rowSums(counts_matched_T_NM >= 10) >= 57 # old method

counts_matched_T_NM_filtered <- counts_matched_T_NM[keep,] 

# xenabrowser method
# counts_matched_T_NM_filtered <- t(scale(t(counts_matched_T_NM)))  # z-Normalize by row (genes)
# low_expression_threshold <- 0.3
# counts_matched_T_NM_filtered <- counts_matched_T_NM_filtered[rowSums(counts_matched_T_NM_filtered) > 0, ]
# mask_low_vals <- rowSums(counts_matched_T_NM_filtered > low_expression_threshold) > 2
# counts_matched_T_NM_filtered <- na.omit(counts_matched_T_NM_filtered[mask_low_vals, ])
###

print(paste0("genes kept: ", nrow(counts_matched_T_NM_filtered)))
#hist(log2(cpm(counts_matched_T_NM_filtered ) + 1), main = "Distribution of log2(CPM+1) values", breaks = 50)
#####

# Use voom to transform the counts
voom_data <- voom(counts_matched_T_NM_filtered, design)
#voom_data <- voom(counts_matched_T_NM_check, design)
...
# Save output
#write.table(TCGA_LUAD_limma_DEG, "../2_Outputs/4_Tumor_DEGs/TCGA_LUAD_limma_DEG_CSFSNS_20250306.txt", sep = '\t')
...
*** Note: In this version I should NOT remove never smokers from the analysis
...
### 3.2.1 Formatting counts in tumor-normal pairs, keeping ALL smoking status
```{r}
...
colnames(data_methylation_hm450_tumor_normal_matched)[1] <- "Gene"
...
### Saving output
#write.table(TCGA_LUAD_limma_DMG, "../2_Outputs/5_Tumor_DMGs/TCGA_LUAD_limma_DMG_CSFSNS_20250225.txt", sep = '\t')
...
v_TM <- EnhancedVolcano::EnhancedVolcano(
 ...
 # pCutoffCol = 'min_smoothed_fdr',
  ...
  widthConnectors = 0.3,
  arrowheads = FALSE,
  pCutoff = FDR_cutoff_TM,
  FCcutoff = log2FC_cutoff_TM,
  gridlines.minor = FALSE,
  gridlines.major = FALSE,
  xlim = c(-3, 3),
  ylim = c(0,10)
