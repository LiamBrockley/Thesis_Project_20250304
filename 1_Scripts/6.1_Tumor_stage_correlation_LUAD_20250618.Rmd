...
#LUAD_A1_TE_tumor_stage_spearman <- read.table("../../2_Outputs/LUAD_A1_TE_tumor_stage_spearman_20250613.txt") 
...
# https://version-12-0.string-db.org/cgi/network?networkId=baLsO0qrek1i
# Despite the fact they are not labelled, CLNK, FCRL2, and MZB1 seem to all be involved in B cells in some way
# They all appear together in this article:
# https://www.sciencedirect.com/science/article/pii/S2090123223000693
# ENRICHR cellmarker 2024 agrees that FCRL2 and MZB1 are markers of B cells in the lung:
...
genes_to_plot <- c("FCRL2","MZB1","CLNK") # All involved in B cells in some way
...
## Save this
# write.table(LUAD_stage_I_comparison, "../../2_Outputs/LUAD_stage_I_comparison_20250618.txt")
...
LUAD_stage_I_comparison <- read.table("../../2_Outputs/LUAD_stage_I_comparison_20250618.txt")

LUAD_stage_I_comparison_filtered <- LUAD_stage_I_comparison %>%
  filter(adj.P.Val <= 0.05)
# STRING: https://version-12-0.string-db.org/cgi/network?networkId=b2MkGu14eQwc

LUAD_stage_I_comparison_filtered_up_up <- LUAD_stage_I_comparison_filtered %>%
  filter(sign(logFC) == 1) %>%
  filter(sign(log2FC_TE) == 1)

LUAD_stage_I_comparison_filtered_down_up <- LUAD_stage_I_comparison_filtered %>%
  filter(sign(logFC) == 1) %>%
  filter(sign(log2FC_TE) == -1)

LUAD_stage_I_comparison_filtered_up_down <- LUAD_stage_I_comparison_filtered %>%
  filter(sign(logFC) == -1) %>%
  filter(sign(log2FC_TE) == 1)

LUAD_stage_I_comparison_filtered_down_down <- LUAD_stage_I_comparison_filtered %>%
  filter(sign(logFC) == -1) %>%
  filter(sign(log2FC_TE) == -1)


LUAD_stage_I_comparison_filtered_A2 <- LUAD_stage_I_comparison_filtered %>%
  filter(rownames(.) %in% LUAD_A1_TE_A2$Gene)

LUAD_stage_I_comparison_filtered_A2_TM <- LUAD_stage_I_comparison_filtered %>%
  filter(rownames(.) %in% LUAD_A1_TE_A2_TM$Gene)

## Make a table to summarize all of these
LUAD_stage_I_comparison_summary <- data.frame(
  gene   = rownames(LUAD_stage_I_comparison_filtered_up_up),
  status = "up/up",
  stringsAsFactors = FALSE
)

LUAD_stage_I_comparison_summary <- rbind(
  LUAD_stage_I_comparison_summary,
  data.frame(
    gene   = rownames(LUAD_stage_I_comparison_filtered_up_down),
    status = "up/down",
    stringsAsFactors = FALSE
  )
)
...
LUAD_stage_I_comparison_summary <- rbind(
  LUAD_stage_I_comparison_summary,
  data.frame(
    gene   = rownames(LUAD_stage_I_comparison_filtered_down_down),
    status = "down/down",
    stringsAsFactors = FALSE
...
LUAD_stage_I_comparison_summary$A2 <- LUAD_stage_I_comparison_summary$gene %in% rownames(LUAD_stage_I_comparison_filtered_A2)

LUAD_stage_I_comparison_summary$A2_TM <- LUAD_stage_I_comparison_summary$gene %in% rownames(LUAD_stage_I_comparison_filtered_A2_TM)

# Save this summary list
write.table(LUAD_stage_I_comparison_summary, "../../2_Outputs/LUAD_stage_I_comparison_summary_20250829.txt")
...
#gene_symbol <- "C4BPA" # Largest negative log2FC
gene_symbol <- "KRT6A" # Largest positive log2FC
#gene_symbol <- "MAOB"
...
label_data <- LUAD_stage_I_comparison[rownames(LUAD_stage_I_comparison)==gene_symbol,]
...
ggplot(plot_data, aes(x = Stage_grouped, y = Expression, fill = Stage_grouped)) +
  ...
  labs(title = gene_symbol,
       x = "LUAD Tumor Stage", y = "logCPM Expression") +
  theme_minimal(base_size = 14) +
...
gene_symbols <- c("CLNK", "FCRL2", "MZB1")  # Add as many as you want

# Match column names once
matched_samples <- intersect(colnames(logCPM), clinical_data_filtered_early_late$bcr_patient_barcode)

# Rebuild plot_data
plot_data <- do.call(rbind, lapply(gene_symbols, function(gene_symbol) {
  gene_id <- gene_ids[gene_symbol]
  
  # Skip if gene_id not in logCPM
  if (is.na(gene_id) || !(gene_id %in% rownames(logCPM))) return(NULL)
  ...
  data.frame(
    Gene = gene_symbol,
    Expression = as.numeric(expr),
    Stage_grouped = clinical_data_filtered_early_late[match(matched_samples, clinical_data_filtered_early_late$bcr_patient_barcode), "Stage_grouped"],
...
ggplot(plot_data, aes(x = Stage_grouped, y = Expression, fill = Stage_grouped)) +
  ...
  facet_wrap(~ Gene, scales = "free_y") +
  labs(title = "Expression of Selected Genes by LUAD Tumor Stage",
