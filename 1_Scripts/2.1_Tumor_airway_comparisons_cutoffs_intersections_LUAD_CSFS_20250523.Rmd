...
2025/05/23:
Here I am using the sequential filtering method because the last two filtering steps generate more overlap than expected by chance via the Fisher's exact test.

This is the method I was using in a sequential pipeline to try to test each assumption being made along the way. I arbitrarily picked a 75th percentile log2FC cutoff -- the test results were the same (significant) even with a much less stringent cutoff, interestingly.
...
LUAD_TM_formatted <- TCGA_LUAD_DMG %>%
...
I don't think that the hypergeometric test can apply to checking the A2 overlap step, because A1 and A2 are not truly independent. Since they both measure genes with expression affected by smoking in airways (A1 in current smokers and A2 in former smokers), they will be expected to share many genes.
...
LUAD_TM_sig <- LUAD_TM_formatted %>%
  filter(FDR_TM <= 0.05)

# Then filter for 75th percentile
top_cutoff_TM <- quantile(LUAD_TM_sig$log2FC_TM, 0.875, na.rm = TRUE)  # Top 12.5%
bottom_cutoff_TM <- quantile(LUAD_TM_sig$log2FC_TM, 0.125, na.rm = TRUE)  # Bottom 12.5%

# Filter genes in the top and bottom 25% of log2FC
LUAD_TM_sig_percentile <- LUAD_TM_sig %>%
  filter(log2FC_TM >= top_cutoff_TM | log2FC_TM <= bottom_cutoff_TM)
nrow(LUAD_TM_sig_percentile)

LUAD_A1_TE_A2_TM <- LUAD_A1_TE_A2 %>%
  dplyr::inner_join(., LUAD_TM_sig_percentile, by = "Gene") %>%
  dplyr::filter(sign(log2FC_A1) != sign(log2FC_TM)) # Ensure signs are opposite
nrow(LUAD_A1_TE_A2_TM)

LUAD_A1_TE_A2_TM$Gene

## Save 
#writeLines(LUAD_A1_TE_A2_TM$Gene, "../../2_Outputs/4_Linked_genes/LUAD_A1_TE_A2_TM_genes_20250528.txt")
write.table(LUAD_A1_TE_A2_TM, "../../2_Outputs/4_Linked_genes/LUAD_A1_TE_A2_TM_20250610.txt")
...
I want to try Dr. DeBoer's suggestion of linear correlations and PCA.
